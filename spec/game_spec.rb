...
describe Game do
  ...
  let(:player_1) { Player.new("Ant") }
  let(:player_2) { Player.new("John") }
  ...
  context 'Single player game' do
    ...
    describe "#players" do
      it "returns the single player's name" do
        expect(single_player_game.player_1.name).to eq "Ant"
      end
    end
  ...
  context 'multiplayer game' do
    it "returns player_2's name" do
      expect(multi_player_game.player_2.name).to eq "John"
    end
