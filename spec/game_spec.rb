...
describe Game do
  ...
  describe '#give_weapon' do
    it 'returns whichever weapon was chosen' do
      expect(solo_game.give_weapon(player_red, 1)[:imp]).to eq :paper
    end

    it 'calls receive_weapon on whichever player is chosen to receive' do
      expect(player_red).to receive(:receive_weapon).with({ :imp => :paper, :winv => [:rock] })
      solo_game.give_weapon(player_red,1)
    end

    it 'gives a random weapon if no weapon is specified' do # [srand = rand]: [1,1], [2,0], [3,2]
      expect_weapon({ :imp => :paper, :winv => [:rock] }, 1)
      expect_weapon({ :imp => :rock, :winv => [:scissors] }, 2)
      expect_weapon({ :imp => :scissors, :winv => [:paper] }, 3)
    end

    def expect_weapon(imp, n)
      srand(n)
      expect(player_red).to receive(:receive_weapon).with(imp)
      solo_game.give_weapon(player_red)
    end
  end

  let(:player_with_rock) { double("A player with a rock", :weapon => { :imp => :rock, :winv => [:scissors] }) }
  let(:player_with_scissors) { double("A player with scissors", :weapon => { :imp => :scissors, :winv => [:paper] }) }
  ...
  describe '#make_choice' do
    it 'calls give_weapon on player 1 for turn 1, or player 2 for turn 2' do
      ...
      expect(player_blue).to receive(:receive_weapon).with({ :imp => :paper, :winv => [:rock] })
      multi_game.make_choice(1)
    end
