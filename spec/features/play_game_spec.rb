...
feature 'Play rock/paper/scissors' do
  context 'one player game' do
    scenario 'allows to choose option' do
      register_one_player_and_play('rock')
      expect(page).to have_content 'You have chosen rock'
    end
    context 'return correct results of a game' do
      scenario 'can win' do
        allow_any_instance_of(Array).to receive(:sample).and_return(:rock)
        register_one_player_and_play('paper')
  ...
  context 'two players game' do
    scenario 'allows to choose option' do
      register_two_players_and_play('rock', 'scissors')
