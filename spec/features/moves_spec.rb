feature 'Game moves' do
  context 'Single player tests' do
    before(:each) { sign_in_and_play }
    scenario 'Does not crash in sp if no move is entered' do
      click_button('Go!') # will crash and fail if buggy
      expect(page).to have_content('0 - 0')
    end
  end
  context 'Multiplayer tests' do
    ...
    scenario '#first_round' do
      expect(page).to_not have_content('Jack used')
    end
    describe 'Rock' do
