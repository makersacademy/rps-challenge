...
describe 'WinnerCalculator' do
  ...
  describe '#calculate' do
    context 'when player 1 wins' do
      it 'calls last move on player 1' do
        allow(player_two).to receive(:last_move).and_return(:scissors)
        expect(player_one).to receive(:last_move).twice
        winner_calculator.calculate(player_one, player_two)
      end

      it 'calls last_move on player 2' do
        allow(player_one).to receive(:last_move).and_return(:rock)
        expect(player_two).to receive(:last_move).twice
        winner_calculator.calculate(player_one, player_two)
      ...
      it 'returns :player_one for paper > rock' do
        allow(player_one).to receive(:last_move).and_return(:paper)
        allow(player_two).to receive(:last_move).and_return(:rock)
        expect(winner_calculator.calculate(player_one, player_two)).to be(player_one)
      end
      it 'returns :player_one for scissors > paper' do
        allow(player_one).to receive(:last_move).and_return(:scissors)
        allow(player_two).to receive(:last_move).and_return(:paper)
        expect(winner_calculator.calculate(player_one, player_two)).to be(player_one)
      end
    end
